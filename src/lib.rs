...
pub const ANSI_REST: &str = "\x1b[0m";
...
macro_rules! solve {
    ($part:expr, $solver:ident, $input:expr) => {{
        ...
        fn print_result<T: Display>(func: impl FnOnce(&str) -> Option<T>, input: &str) {
            ...
            match result {
                Some(result) => {
                    println!(
                        "{} {}(elapsed: {:.2?}){}",
                        result, ANSI_ITALIC, elapsed, ANSI_REST
                    );
                }
                None => {
                    println!("not solved.");
        ...
        println!("{} Part {}{}", ANSI_BOLD, $part, ANSI_REST);
...
pub mod aoc_cli {
    ...
    pub fn download(day: u8, year: Option<u16>) -> Result<Output, AocCliError> {
        ...
        create_dir_all("src/puzzles").map_err(|_| AocCliError::CommandNotFound)?;
